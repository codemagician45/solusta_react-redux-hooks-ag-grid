...
const ImagePart = ({ item }) => {
  switch (item.attendeeCategorySAS[0].categoryName) {
    case "Speaker": {
      return <img src={BG3} alt="background" />;
    }
    case "Organizer": {
      return <img src={BG5} alt="background" />;
    }
    case "Participant": {
      return <img src={BG1} alt="background" />;
    }
    case "Event Crew": {
      return <img src={BG6} alt="background" />;
    }
    case "Media": {
      return <img src={BG2} alt="background" />;
    }
    case "Security": {
      return <img src={BG4} alt="background" />;
    }
    case "Contractor": {
      return <img src={BG7} alt="background" />;
    }
    default: {
      return <img src={BG7} alt="background" />;
    }
...
class PrintComponent extends React.Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (
      nextProps.attendees !== prevState.attendees ||
      nextProps.selectedRows !== prevState.selectedRows
    ) {
      return {
  ...
  getFriendlyId = item => {
    return new Promise((resolve, reject) => {
      Utils.xapi()
        .get(`/badge-sas?attendeeSAId.equals=${item.id}`)
        .then(res => {
          console.log("here in friendlyID: ", res);
          resolve(
            res.data && res.data.length > 0 ? res.data[0].badgeFriendlyID : 0
          );
        })
        .catch(err => {
          reject(err);
            ...
            return (
              ...
              >
                <div id="modal-print" className={classes.modal_print}>
                  <h1 className={classes.nameStyle}>
                    {item.firstName + " " + item.lastName}
                  </h1>
                  <h2 className={classes.companyNameStyle}>
                    {item.companyName}
                  </h2>
                  {fId && fId[0] && (
                  ...
                  <div className={classes.photoContainer}>
                    <img
                      ...
                      src={`data:${item.mainPhotoContentType};base64, ${item.mainPhoto}`}
                      alt="badge"
                    />
                  </div>
                </div>
